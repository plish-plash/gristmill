...
impl<'a> WindowSetup<'a> {
    pub fn with_title(title: String) -> Self {
        WindowSetup { title, icon: None, resizable: true, config: Path::new("window.yaml") }
    }
    pub fn with_title_and_icon(title: String, icon: &'a Path) -> Self {
        WindowSetup { title, icon: Some(icon), resizable: true, config: Path::new("window.yaml") }
    }
}
...
impl Default for WindowConfig {
    fn default() -> Self {
        Self { width: 800, height: 600, fullscreen: false, sample_count: None }
    }
}
...
impl From<WindowIcon> for Icon {
    fn from(value: WindowIcon) -> Self {
        ...
        assert_eq!(value.0.len(), SMALL_SIZE + MEDIUM_SIZE + LARGE_SIZE, "WindowIcon invalid data");
        Icon { small: value.0[0..SMALL_SIZE].try_into().unwrap(), medium: value.0[SMALL_SIZE..(SMALL_SIZE+MEDIUM_SIZE)].try_into().unwrap(), big: value.0[(SMALL_SIZE+MEDIUM_SIZE)..].try_into().unwrap() }
    }
}
...
pub(crate) fn load_config(window_setup: WindowSetup, default_config: WindowConfig) -> Conf {
    let icon = window_setup.icon.and_then(|path| asset::load_file(path).and_then(|mut reader| {
        use std::io::Read;
        let mut icon = WindowIcon(Vec::new());
        reader.read_to_end(&mut icon.0).map_err(|e| AssetError::new_io(path.to_owned(), false, e))?;
        Ok(icon)
    }).inspect_err(|e| log::error!("{}", e)).ok());
    let config = WindowConfig::load(window_setup.config).inspect_err(|e| log::warn!("{}", e)).unwrap_or(default_config);
...
pub fn get_fullscreen() -> bool {
    let config = GLOBAL_CONFIG.lock().unwrap();
    config.as_ref().map(|(_, config)| config.fullscreen).unwrap_or_default()
}
